...
# question 5
cur.execute("""
SELECT mainauthor.id as id, mainauthor as author1, coauthor as author2 into table temp_table FROM mainauthor inner join coauthors on mainauthor.id = coauthors.id where mainauthor.mainauthor != '' order by mainauthor.id;
""")

cur.execute("""
  insert into temp_table select c1.id as id, c1.coauthor as author1, c2.coauthor as author2 from coauthors as c1 inner join coauthors as c2 on c1.id = c2.id where c1.coauthor != c2.coauthor;
""")

print(pd.read_sql("SELECT * from temp_table;", conn))
